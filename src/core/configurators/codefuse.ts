...
/**
 * Cod AI Tool Configurator
 * 
 * Configures OpenSpec integration for Cod AI coding assistant.
 * Creates and manages CODEFUSE.md configuration file with OpenSpec instructions.
 * 
 * @implements {ToolConfigurator}
 */
export class CodeFuseConfigurator implements ToolConfigurator {
  /** Display name for the CodeFuse tool */
  name = 'CodeFuse';
  
  /** Configuration file name at project root */
  configFileName = 'CODEFUSE.md';
  
  /** Indicates tool is available for configuration */
  isAvailable = true;

  /**
   * Configure Codefuse integration for a project
   * 
   * Creates or updates CODEFUSE.md file with OpenSpec instructions.
  ...
  async configure(projectPath: string, openspecDir: string): Promise<void> {
    // Construct full path to CODEFUSE.md at project root
    const filePath = path.join(projectPath, this.configFileName);
    
    // Get Claude-compatible instruction template
    // This ensures Cod receives the same high-quality OpenSpec instructions
